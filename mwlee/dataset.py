...
class LoadDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        if image.size[0] < image.size[1] : img = transforms.RandomRotation((-90, -90), expand = True)(image)
...
class LoadEvalDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        image = np.array(image)
        #image = A.transforms.PadIfNeeded(min_height = 512, min_width = 512, border_mode = 1, p = 1)(image = image)["image"]
        #image = A.CLAHE(p=1)(image = image)["image"]
        image = claheCVT(image)
        image = transforms.ToPILImage()(image)
        
        if image.size[0] < image.size[1] : image = transforms.RandomRotation((-90, -90), expand = True)(image)
