...
#             print('what is decoded_values',decoded_values.shape,decoded_values)            # pseudo 할 때 쓸거
            score, sequence = torch.topk(decoded_values, 1, dim=1)
#             print('what is sequence',sequence.shape,sequence)
#             print('before sq',sequence.shape,sequence)
            sequence = sequence.squeeze(1)
            score = score.squeeze(1)
            score=torch.log(score)
            score=torch.mean(score,1,True).tolist()
#             print('after sq',sequence.shape,sequence)
#             print('dec',decoded_values.shape,decoded_values)
            ...
            if sentence_acc(sequence_str,expected_str)>0.8 and train==False:
                print('validation high enough, do train.')
                PL.append(d)
#             if not train:         # val의 정보 출력
#                 print(d)
            ...
            wandb.log({"loss": np.mean(losses), 
                      ...
                      "ratio" : correct_symbols/total_symbols,
                      "wer" :wer,
                      "sent_acc" : sentence_acc(sequence_str,expected_str),
                      "score" : np.mean(score)})
...
#             A.augmentations.geometric.resize.LongestMaxSize (max_size=128,  p=1),
#             A.augmentations.transforms.PadIfNeeded (min_height=128, min_width=128, border_mode=1, p=1.0),
#             ToTensorV2()
#             T.functional.adjust_sharpness(sharpness_factor=2) ,
            T.Resize((options.input_size.height, options.input_size.width)),
            T.ToTensor(),
    ...
    for epoch in range(options.num_epochs):
        ...
        # Train
#         print('tfratio',(options.teacher_forcing_ratio-(epoch/options.num_epochs)*options.teacher_forcing_ratio_drop))
#         print(train_dataset[0])
#         print(len(train_dataset))
#         print(validation_sentence_accuracy[-1])
        if validation_sentence_accuracy:
            val_acc=max(validation_sentence_accuracy)
        else :
            val_acc=0
...
#             options.teacher_forcing_ratio,
            1-val_acc,
#             (options.teacher_forcing_ratio-(epoch/options.num_epochs)*options.teacher_forcing_ratio_drop),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--config_file",
        dest="config_file",
        default= "configs/SATRN_All.yaml",
        type=str,
        help="Path of configuration file",
    )
    wandb.login()
    wandb.init(project = 'OCR', ##프로젝트 이름
    name = 'self+PL+rot_learn') ##run 이름)
