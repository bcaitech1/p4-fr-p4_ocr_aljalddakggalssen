...
class CNN(nn.Module):
    def __init__(self, nc, device, leakyRelu=False):
...
class AttentionCell(nn.Module):
    def __init__(self, src_dim, hidden_dim, embedding_dim, device, num_layers=1,
         cell_type='LSTM'):
...
class AttentionDecoder(nn.Module):
    def __init__(
        ...
        self.attention_cell = AttentionCell(
            src_dim, hidden_dim, embedding_dim, 
            num_layers=num_layers, cell_type=cell_type, device=device
        )
        self.hidden_dim = hidden_dim
    ...
    def forward(
        ...
        if self.num_layers == 1:
            hidden = (
                torch.FloatTensor(batch_size, self.hidden_dim).fill_(0).to(self.device),
                torch.FloatTensor(batch_size, self.hidden_dim).fill_(0).to(self.device),
            )
        else:
            hidden = [
        ...
        if is_train and random.random() < teacher_forcing_ratio:
            ...
            targets = (
                torch.LongTensor(batch_size).fill_(self.st_id).to(self.device)
            )  # [START] token
            probs = (
...
class Attention(nn.Module):
    def __init__(
        ...
        self.encoder = CNN(FLAGS.data.rgb, device=device)
